...
# ---------------------------------------------------------------------------
# xterm.js static assets – served from separate files in src/webui/
# (@xterm/xterm 6.0.0  +  @xterm/addon-fit 0.11.0)
# See: src/webui/xterm.js, src/webui/xterm.css, src/webui/addon-fit.js
# ---------------------------------------------------------------------------

# Directory searched for xterm static assets at runtime.
# Installed layout:  /usr/share/modernjvs/
# In-source layout:  <repo>/src/webui/
_WEBUI_STATIC_DIR = (
    "/usr/share/modernjvs"
    if os.path.isdir("/usr/share/modernjvs")
    else os.path.join(os.path.dirname(os.path.abspath(__file__)))
)

# ---------------------------------------------------------------------------
# HTML / CSS / JS – HTML is embedded; xterm JS/CSS served from static files
# ---------------------------------------------------------------------------
...
class WebUIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _serve_static_file(self, filename, content_type):
        path = os.path.join(_WEBUI_STATIC_DIR, filename)
        try:
    ...
    def _serve_xterm_js(self):
        self._serve_static_file("xterm.js", "application/javascript")

    def _serve_xterm_css(self):
        self._serve_static_file("xterm.css", "text/css")

    def _serve_addon_fit_js(self):
        self._serve_static_file("addon-fit.js", "application/javascript")
