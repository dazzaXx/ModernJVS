...
ModernJVS ships a self-contained test suite in `tests/test_modernjvs.c` that exercises the core protocol logic **without requiring any real hardware** (the RS485 serial port is emulated with a Unix `socketpair`).

### Building the tests

The test binary is built automatically as part of a normal development build (`BUILD_TESTS` is `ON` by default):

```bash
mkdir -p build && cd build && cmake .. && make
```

This produces `build/test_modernjvs` alongside the main `build/modernjvs` binary.

To **skip** building the tests (e.g. for a production/install build):

```bash
cd build && cmake .. -DBUILD_TESTS=OFF && make
```

### Running the tests

**Run directly** — prints a line per test and a pass/fail summary:

```bash
./build/test_modernjvs
```
...
```
========================================
  ModernJVS Test Suite
========================================

  PASS  test_initIO_zeros_state
  PASS  test_setSwitch_system
  ...
  PASS  test_processPacket_namco_specific_program_date
...
The binary exits `0` on full pass and `1` if any test fails.

**Run via CTest** (useful for CI or when you want formatted output):
...
| Group | Modules | What's covered |
|-------|---------|----------------|
| IO state | `jvs/io.c` | `initIO`, `setSwitch` (system/player/range checks), `incrementCoin`, `setAnalogue`, `setGun` (axis, inversion, range), `setRotary`/`getRotary`, `jvsInputFromString`, `jvsPlayerFromString` |
| Config parsing | `console/config.c` | `getDefaultConfig`, `parseConfig` (valid files, missing files, comments, `INCLUDE`, deadzone & WII_IR_SCALE clamping), `parseIO` against real installed IO files |
| Debug filtering | `console/debug.c` | Level get/set, output suppression at each level |
| JVS packet framing | `jvs/jvs.c` | `readPacket` (valid, bad checksum, escaped `0xE0`/`0xD0`, SYNC re-sync, timeout), `writePacket` (basic, length not permanently modified, short packet, escaped data) |
| `processPacket` integration | `jvs/jvs.c` | CMD_RESET, CMD_ASSIGN_ADDR, CMD_REQUEST_ID, CMD_CAPABILITIES, CMD_READ_SWITCHES/COINS/ANALOGS/ROTARY/LIGHTGUN/GPI, CMD_DECREASE_COINS, CMD_WRITE_COINS/GPO, CMD_CONVEY_ID, CMD_REMAINING_PAYOUT, CMD_SET_PAYOUT, CMD_RETRANSMIT, multi-command packets, chained address assignment, not-for-us packet filtering, connection timeout, CMD_NAMCO_SPECIFIC |

### Adding new tests

All tests live in `tests/test_modernjvs.c`. The file contains a minimal test framework (`TEST_BEGIN`, `ASSERT`, `TEST_PASS`, `FAIL`) — add a new `static void test_your_feature(void)` function and register it in the `tests[]` array near the bottom of the file.
